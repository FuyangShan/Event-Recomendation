...
public class TicketMasterClient {
	...
	private static final String API_KEY = "bAKRSh5K02Q60FF1RMXVY1x7edFxXxbA";
	
	//servelet 来调用search fucntion
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			String line;
			while ((line = reader.readLine()) != null) { //只要有数据源就继续读
		...
		try {
			// Extract events array only. HTTP请求一般都是JSON来进行的
			JSONObject obj = new JSONObject(responseBuilder.toString());
			if (!obj.isNull("_embedded")) { //我们需要的数据都在"_embedded"（从TM的API doc里边） ，这里在处理error
	...
	private String getImageUrl(JSONObject event) throws JSONException {
		if (!event.isNull("images")) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if (!image.isNull("url")) { //有的数据有可能没有图片
